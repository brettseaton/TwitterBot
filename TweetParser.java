...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        String[] arrOfCSV = csvLine.split(",");
        if (arrOfCSV.length <= csvColumn || csvColumn < 0) {
            return null;
        }
        return arrOfCSV[csvColumn];
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        FileLineIterator newFLI = new FileLineIterator(br);
        List<String> tweetList = new LinkedList<>();
        String iterable = "";
        while (newFLI.hasNext()) {
            iterable = extractColumn(newFLI.next(), tweetColumn);
            // no try catch necessary here
            if (iterable != null) {
                tweetList.add(iterable);
            }
        }
        return tweetList;
    }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        String[] wordCleanArray = sentence.split(" ");
        List<String> listOfCleanWords = new LinkedList<String>();

        for (String s : wordCleanArray) {
            if (cleanWord(s) != null) {
                listOfCleanWords.add(cleanWord(s));
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<List<String>> parseAndCleaned = new LinkedList<>();
        List<String> oneTweet = tweetSplit(removeURLs(tweet));
        for (String dirtySentence : oneTweet) {
            List<String> added = parseAndCleanSentence(dirtySentence);
            if (!added.isEmpty()) {
                parseAndCleaned.add(added);
            }
        }
        return parseAndCleaned;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        String tweet = "";
        List<String> listOfStringsFromCSV = csvDataToTweets(br, tweetColumn);
        for (String str : listOfStringsFromCSV) {
            tweet += str + ".";
        }
        return parseAndCleanTweet(tweet);
    }
