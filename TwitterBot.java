...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        //add buffered and tweetCol to data
        List<List<String>> csvData = TweetParser.csvDataToTrainingData(br, tweetColumn);

        for (List<String> quickList : csvData) {
            mc.train(quickList.iterator());
        }
    }
    ...
    public void writeStringsToFile(
        ...
        try {
            BufferedWriter bufferedWrite = new BufferedWriter(new FileWriter(file, append));
    ...
    public String generateTweet(int numWords) {
        ...
        if (numWords < 0) {
            throw new IllegalArgumentException("Can't have negative number of words!");
        }
        ...
        return (retVal + randomPunctuation());
    }
